...
from lib.db.connection import get_connection  # Fixed import here
import sqlite3
import pytest
import os
...
if not callable(globals().get("get_connection")):
    from lib.db.connection import get_connection  # Fixed import here too
...
def clean_up(tbl):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(f"DELETE FROM {tbl}")  # fixed query - use f-string not parameter here for table name
    conn.commit()
    conn.close()
...
def count(tbl, where):
    conn = get_connection()
    cursor = conn.cursor()
    q = "SELECT COUNT(*) AS count FROM %s WHERE %s = ?" % (tbl, list(where.keys())[0])
    row = cursor.execute(q, tuple(where.values())).fetchone()
    conn.close()
    return row["count"]
...
def test_author_save_and_find():
    ...
    assert found is not None and found.name == "Charlie"
